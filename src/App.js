...
var People = React.createClass({
  ...
  getInitialState: function() {
    if (localStorage.getItem("saved") === "true") {
      return {data: [], noSave: true, justSaved: false}
    } else if (localStorage.getItem("saved") === "false") {
      return {data: [], noSave: false, justSaved: false}
    }
  },
  ...
  render: function() {
    ...
    staffRows = faculty.map(function(row) {
      return (<PeopleRow fromMe={this.props.fromMe} data={row} saveReady={savePass}/>);
    }.bind(this))
  ...
  render: function() {
    if (this.props.noSave === true) {
      return (<div className="button">
        <Button bsStyle="primary" bsSize="large" disabled="disabled">Save</Button>
      </div>);
    } else {
      return (<div className="button">
        <Button bsStyle="primary" bsSize="large" onClick={this.props.onClick}>Save</Button>
      </div>);
    }
  }
});
...
var Person = React.createClass({
  ...
  render: function() {
    ...
    if (this.state.noReadValue === "true") {
      noReadRender = (<Checkbox
        checked="checked"
        onChange={this.noReadChange}>Don't Read At Ceremony</Checkbox>);
    ...
    return (<div className="person">
      ...
      <textarea defaultValue={this.state.textValue} onChange={this.textareaChange} rows="6" placeholder={this.props.data.placeholder}/> {anonymousRender}
      {noReadRender}
    </div>);
  }
});
...
var NicetyDisplay = React.createClass({
  ...
  render: function() {
    ...
    return (<div className="niceties">
      <Grid>
        {
          list.map(function(row) {
            return (<NicetyRow data={row}/>);
          })
        }
      </Grid>
    </div>);
            ...
            return (<div>
              ...
              <br/> {
                person.niceties.map((nicety) => {
                  return (<AdminNicety nicety={nicety} target_id={person.to_id} admin_edit_api={this.props.admin_edit_api}/>);
                })
              }
              <hr/>
            </div>);
...
var AdminNicety = React.createClass({
  getInitialState: function() {
    ...
    // }
    return {text: this.props.nicety.text, noSave: true, reviewedValue: this.props.nicety.reviewed.toString()};
  },
  // saveReady: function () {
  //     this.setState({noSave: false});
  // },
  ...
  render: function() {
    ...
    if (this.props.nicety.text !== '' && this.props.nicety.text !== null) {
      ...
      nicetyReturn = (<div>
        ...
        <h5>{noRead}</h5>
        <textarea defaultValue={this.state.text} onChange={this.textareaChange} rows="6" style={textStyle}/>
...
var App = React.createClass({
  ...
  getInitialState: function() {
    return {fromMe: [], people: [], niceties: [], currentview: "write-niceties", selfInfo: []};
  },
  componentDidMount: function() {
      ...
      this.loadPeopleFromServer(function(data2) {
        this.loadNicetiesForMe(function(data3) {
          this.loadSelfInfo(function(data4) {
            this.setState({niceties: data3, people: data2, fromMe: data1, selfInfo: data4});
          }.bind(this));
        }.bind(this));
      }.bind(this));
    }.bind(this));
  ...
  selectComponent: function(idx) {
    switch (idx) {
      case "write-niceties":
        ...
        return <People people={this.state.people} fromMe={this.state.fromMe} save_nicety_api={this.props.save_nicety_api}/>
      case "view-niceties":
